 ...
 * YouTube, y el proceso principal le añade la cabecera de origen sin la que
 ...
 * reanudan a trompicones, porque el clic en "Ver tráiler" ocurre en la app y no
...
export function TrailerPlayer({ youtubeKey, title, onClose }: Props): JSX.Element {
  useEffect(() => {
    const onKey = (event: KeyboardEvent): void => {
      if (event.key === 'Escape') {
        // La ficha de debajo también escucha Escape: esta tecla es solo nuestra.
        event.stopImmediatePropagation()
        onClose()
      }
    }
    window.addEventListener('keydown', onKey, true)
    // Cuando el foco esta dentro del reproductor, el Escape solo llega por aquí.
  ...
  return (
    <div className="trailer" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      ...
      <button className="trailer-close" onClick={onClose} aria-label="Cerrar tráiler" title="Cerrar (Esc)">
        <IconClose />
      </button>
    </div>
  )
